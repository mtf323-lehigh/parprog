...
main(int argc, char *argv[])  {
  ...
  MPI_Irecv(&buf, 2, MPI_INT, next, tag1, MPI_COMM_WORLD, &reqs[1]);

  // Send 1 to right and 2 to left
  MPI_Isend(&buf, 1, MPI_INT, next, tag2, MPI_COMM_WORLD, &reqs[2]);
  MPI_Isend(&buf, 2, MPI_INT, prev, tag2, MPI_COMM_WORLD, &reqs[3]);
